...
parser.add_argument("--kernel_size", type=int, default=24, help="window size")
parser.add_argument("--strides", type=int, default=24, help="stride size")
...
feature_shapes = dict()
for idx, video_id in enumerate(tqdm(video_ids, desc="Extracting features")):
    ...
    if os.path.exists(image_dir):
        ...
        if args.fps is None or args.fps <= 0:
            ...
            print("ffmpeg -hide_banner -loglevel panic -i {} -filter:v fps=fps={} {}/{}-%6d.jpg".format(video_path, args.fps, image_dir, video_id))
    ...
    for start in range(0, t, args.strides):
        i += 1
        end = min(t - 1, start + args.kernel_size)
        if end - start < args.kernel_size:
            start = max(0, end - args.kernel_size)
